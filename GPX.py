            ...
            if i==0:
              for j in range(len(tsource)):
                if (list(tsource[varml[0]]))[j]<0:
                  print(f"ERROR: The mass of the source at row number {j+1} was found to be {(list(tsource[varml[0]]))[j]}, which is INVALID !!!")
                  print("Mass of the source MUST ALWAYS BE GREATER THAN OR EQUAL TO ZERO !!!",end='\n\n')
                  invalm=1
                  continue
            else:
...
#####################################################################################################################################################################################
    ...
    ########################################################################################################################
          ...
          if svch=='1':
            ...
            if (Table.read(fname,format=fname_fmt))==owtable:
    ...
    ########################################################################################################################
          ...
          # If the user wants to append the result to an existing file
            ...
            if (Table.read(fname,format=fname_fmt))==exrct:
