...
#####################################################################################################################################################################################
  ...
  if ch=='1' or ch=='2':
    ...
    print("2. Load from CSV / FITS File.",end='\n\n')
    print('****************************************************************************************************************************************')
    print('****************************************************************************************************************************************',end='\n\n')
    ...
    if chmc=='1':
      ...
      # Creating NumPy arrays of the source data lists and converting them to Quantity objects with units
      M=np.array(m,dtype=float)*u.kg
      X_M=np.array(x_m,dtype=float)*u.m
      Y_M=np.array(y_m,dtype=float)*u.m
      Z_M=np.array(z_m,dtype=float)*u.m

#####################################################################################################################################################################################

    # If the user wants to load the source data from a csv / fits file
    elif chmc=='2':
      print("NOTE: FILE SEPARATION",end='\n\n')
      print('****************************************************************************************************************************************',end='\n\n')
      print("> You are about to load source data from a CSV / FITS file.")
      print("> Use TWO SEPARATE CSV / FITS FILES - one for source masses, one for evaluation points.")
      print("> Combining them in one file may create NULL values and CORRUPT the RESULT.",end='\n\n')
      print('****************************************************************************************************************************************',end='\n\n')

      # Asking user for the csv / fits file containing the source data
      fsource=input("Enter the filename (including .csv / .fits extension) containing source mass and position data: ")
      print('\n')

      # Ensuring the file has a .csv / .fits extension
      if not ((fsource.lower().endswith('.csv')) or (fsource.lower().endswith('.fits'))):
        print("This program ONLY SUPPORTS .csv / .fits file format !!!")
        continue

      # Setting table format based on file extension (.csv / .fits)
      if (fsource.lower().endswith('.csv'))==True:
        fsource_fmt='csv'
      else:
        fsource_fmt='fits'
      ...
      try:
        tsource=Table.read(fsource,format=fsource_fmt)
      ...
      print(f"The truncated view of the table extracted from the file {fsource} is:",end='\n\n')
      ...
      for i in range(4):
        chexitm=0                                                               # Variable which would check if the user enters EXIT or to break the while loop
        ...
        while not (chexitm==1 or invalm==1):
          ...
          elif not isinstance(tsource[varml[i]][0],(int,float,np.floating,np.integer)):
          ...
          # If no issue then, break the while loop and return back to the for loop
            ...
            if i==0:
              for j in range(len(tsource)):
                if tsource[varml[0]][j]<0:
                  print(f"ERROR: The mass of the source at row number {j+1} was found to be {tsource[varml[0]][j]}, which is INVALID !!!")
                  print("Mass of the source MUST ALWAYS BE GREATER THAN OR EQUAL TO ZERO !!!",end='\n\n')
                  invalm=1
                  continue
              if invalm==0:
      ...
      # Extracting the data from each column and converting them to NumPy arrays and Quantity objects with units
      M=np.array(tsource[varml[0]],dtype=float)*u.kg
      X_M=np.array(tsource[varml[1]],dtype=float)*u.m
      Y_M=np.array(tsource[varml[2]],dtype=float)*u.m
      Z_M=np.array(tsource[varml[3]],dtype=float)*u.m
...
#####################################################################################################################################################################################
  if ch=='1':
    ...
    while True:
      ...
      print("1b. Discrete Points (CSV / FITS File)  -  Load coordinates from a CSV / FITS file.")
...
#####################################################################################################################################################################################
      ...
      if chev=='1a':
        ...
        nop=only_num("Enter the no.of points where gravitational potential is to be evaluated: ",'int')
        print('\n')
        print('****************************************************************************************************************************************',end='\n\n')
        for i in range(nop):
        ...
        # Creating NumPy arrays and converting them to Quantity objects with units
        X=np.array(x,dtype=float)*u.m
        Y=np.array(y,dtype=float)*u.m
        Z=np.array(z,dtype=float)*u.m

        break

#####################################################################################################################################################################################

      # If the user wants to load the coordinates of evaluation points from a csv / fits file
      elif chev=='1b':
        # Asking user to enter the file name consisting of the coordinates of evaluation points
        feval=input("Enter the filename (including .csv / .fits extension) containing the points for evaluation: ")
        print('\n')

        if not ((feval.lower().endswith('.csv')) or (feval.lower().endswith('.fits'))):
          print("This program ONLY SUPPORTS .csv / .fits file format !!!")
          continue

        if (feval.lower().endswith('.csv'))==True:
          feval_fmt='csv'
        else:
          feval_fmt='fits'

        try:
          teval=Table.read(feval,format=feval_fmt)
        except FileNotFoundError:
        ...
        print(f"The truncated view of the table extracted from the file {feval} is:",end='\n\n')
        print(teval,end='\n\n')
        print('****************************************************************************************************************************************',end='\n\n')
        ...
        for i in range(3):
          ...
          while chexit!=1:
            ...
            elif not isinstance(teval[varl[i]][0],(int,float,np.floating,np.integer)):
        ...
        # Creating NumPy arrays and converting them to Quantity objects with units
        X=np.array(teval[varl[0]],dtype=float)*u.m
        Y=np.array(teval[varl[1]],dtype=float)*u.m
        Z=np.array(teval[varl[2]],dtype=float)*u.m
...
#####################################################################################################################################################################################
      ...
      # User has selected a uniform 3D grid - generate meshgrid from start/terminal points and resolution
        ...
        # Creating equally spaced points along each axis and converting them to Quantity objects with units
        x_points=np.linspace(s[0],e[0],h)*u.m
        y_points=np.linspace(s[1],e[1],h)*u.m
        z_points=np.linspace(s[2],e[2],h)*u.m

        X,Y,Z=np.meshgrid(x_points,y_points,z_points,indexing='ij')         # Creating a uniform 3D grid
...
#####################################################################################################################################################################################
    ...
    # Initialize P as a Quantity array with units of potential (J/kg or m^2/s^2)
    P = np.zeros(X.shape) * (u.J / u.kg)
    ...
    # Convert epsilon to a Quantity object with units of meters
    eps_qty = eps * u.m

    # Calculating the potential for each point by each source mass and appending it to P
    for i in range(len(M)):
      # All variables in the calculation are now Quantity objects, ensuring correct unit propagation
      p=(-(G)*(M[i]))/((X-X_M[i])**2+(Y-Y_M[i])**2+(Z-Z_M[i])**2+(eps_qty)**2)**(0.5)
      P+=p
    ...
    # r_eval will now correctly be a Quantity array with units of meters
    r_eval=np.sqrt((X**2)+(Y**2)+(Z**2))

    # astropy.table.Table can handle Quantity objects directly, retaining unit information
    output['Sr. No.']=np.arange(1,(len(np.ravel(X)))+1,1)
...
#####################################################################################################################################################################################
    ...
    while True:
      ...
      print("4. Save result table to a CSV / FITS file.")
...
#####################################################################################################################################################################################
      ...
      # If the user wants to know the summary of the potential evaluation
        ...
        # Extract values for min/max calculations if they are Quantity objects
        RX,RY,RZ,RP=np.ravel(X.value),np.ravel(Y.value),np.ravel(Z.value),np.ravel(P.value)
        ...
        print('****************************************************************************************************************************************',end='\n\n')
        print(f"Minimum gravitational potential was evaluated to be: {minp} {P.unit} and at a distance of {min_reval} {X.unit} from the origin.")
        print("Coordinate(s) at which this occurs:",end='\n\n')
        for k in range(len(mincood)):
          print(f"{k+1}) {mincood[k]} {X.unit}") # Adding units to coordinates for clarity
        print('****************************************************************************************************************************************',end='\n\n')
        print(f"Maximum gravitational potential was evaluated to be: {maxp} {P.unit} and at a distance of {max_reval} {X.unit} from the origin.")
        print("Coordinate(s) at which this occurs:",end='\n\n')
        for k in range(len(maxcood)):
          print(f"{k+1}) {maxcood[k]} {X.unit}")
        print('****************************************************************************************************************************************',end='\n\n')
...
#####################################################################################################################################################################################
    ...
    ########################################################################################################################
        ...
        if svch=='1':
          # Asking user for a name of the new file to be created / existing file to be overwritten (in .csv / .fits): ")
          fname=input("Enter the name of the new file to be created / existing file to be overwritten (in .csv / .fits): ")
          print('\n')
          ...
          print("> These names will appear in the CSV / FITS file.")
          ...
          # Creating a new table and appending the data into it, passing Quantity objects directly
          owtable=Table()
          owtable['Sr. No.']=np.arange(1,(len(np.ravel(X)))+1,1)
          ...
          owtable.write(fname,format=fname_fmt,overwrite=True)
    ...
    ########################################################################################################################
        ...
        # If the user wants to append the result to an existing file
          ...
          # Asking user to enter the name of an existing file
          fname=input("Enter the name of an existing file (in .csv / .fits): ")
          print('\n')
          ...
          # Ensuring that the file provided exists
          try:
            exrct=Table.read(fname,format=fname_fmt)
          except FileNotFoundError:
            print(f"The filename {fname} provided does NOT EXIST !!! Please ENSURE that the file names provided do EXIST !")
            continue

          # Ensuring no column mismatch
          if len(exrct)!=len(output):
          ...
          # Printing the extracted table
          print(f"The truncated view of the table extracted from the file {fname} provided is:",end='\n\n')
          print(exrct,end='\n\n')
          print('****************************************************************************************************************************************',end='\n\n')
          ...
          exrct.write(fname,format=fname_fmt,overwrite=True)
    ...
    ########################################################################################################################
        ...
        # If the user goes with an invalid choice from the save menu
           ...
           print("INVALID choice from the SAVE SUBMENU !!!",end='\n\n')
...
#####################################################################################################################################################################################
      ...
      if vizch=='1' or vizch=='2' or vizch=='3':
        ...
        if constaxis=='x':
          defaxis('X','Y','Z')
          # Convert XPLOT to Quantity object with units
          XPLOT=only_num("Enter the value of X where the slice is to be taken (X = constant): ", 'float')*u.m
          dispaxes=['Y','Z']
          ...
          defaxis('Y','X','Z')
          # Convert YPLOT to Quantity object with units
          YPLOT=only_num("Enter the value of Y where the slice is to be taken (Y = constant): ", 'float')*u.m
          dispaxes=['X','Z']
          ...
          defaxis('Z','X','Y')
          # Convert ZPLOT to Quantity object with units
          ZPLOT=only_num("Enter the value of Z where the slice is to be taken (Z = constant): ", 'float')*u.m
          dispaxes=['X','Y']
        ...
        # Generate equally spaced grid points for horizontal and vertical axes and convert to Quantity objects
        varaxes[0]=np.linspace(s[0],e[0],h)*u.m
        varaxes[1]=np.linspace(s[1],e[1],h)*u.m

        # Perfroming a meshgrid to create 2D coordinate matrices required by contour/contourf (will preserve units)
        VARAXES[0],VARAXES[1]=np.meshgrid(varaxes[0],varaxes[1],indexing='ij')

        # Initialize GPPLOT as a Quantity array with units of potential
        GPPLOT=np.zeros(VARAXES[0].shape) * (u.J / u.kg)
        ...
        # Convert epsilon to a Quantity object with units
        eps_qty = eps * u.m

        # Assign VARAXES[0] and VARAXES[1] to XPLOT, YPLOT, ZPLOT based on constaxis dynamically
        if constaxis == 'x':
            YPLOT_eval, ZPLOT_eval = VARAXES[0], VARAXES[1]
            XPLOT_eval = XPLOT # Already a Quantity
        elif constaxis == 'y':
            XPLOT_eval, ZPLOT_eval = VARAXES[0], VARAXES[1]
            YPLOT_eval = YPLOT # Already a Quantity
        else: # constaxis == 'z'
            XPLOT_eval, YPLOT_eval = VARAXES[0], VARAXES[1]
            ZPLOT_eval = ZPLOT # Already a Quantity

        # Calculating the potential for each point by each source mass and appending it to GPPLOT
        for i in range(len(M)):
          # The calculation is now fully unit-aware
          dist_sq = (XPLOT_eval - X_M[i])**2 + (YPLOT_eval - Y_M[i])**2 + (ZPLOT_eval - Z_M[i])**2 + (eps_qty)**2
          gpplot = (-(G)*(M[i]))/(dist_sq**(0.5))
          GPPLOT+=gpplot
        ...
        if vizch=='1' or vizch=='3':
          # Use the unit from GPPLOT for the colorbar label
          cblabel=input(f"Label for colorbar (e.g., 'Potential ({GPPLOT.unit})'): ")

        # Constructing the graph according to user's choice
        # Matplotlib functions can handle Quantity objects, but it's often safer to pass .value if units are not explicitly supported
        if vizch=='1':
          graph=plt.imshow(GPPLOT.value,cmap='magma',origin='lower',extent=[s[0],e[0],s[1],e[1]])
        elif vizch=='2':
          graph=plt.contour(VARAXES[0].value,VARAXES[1].value,GPPLOT.value,levels=lvl,cmap='magma')
        else:
          graph=plt.contourf(VARAXES[0].value,VARAXES[1].value,GPPLOT.value,levels=lvl,cmap='magma')

        # Displaying colorbar or labels depending on plot type
        if vizch=='1' or vizch=='3':
          plt.colorbar(graph,label=cblabel)
        else:
          # Adjust clabel to display units if needed, or format values appropriately
          plt.clabel(graph,inline=False,fontsize=8,fmt=lambda x: f'{x:.1f} {GPPLOT.unit}')

        # Setting plot title and axis labels
        plt.title(ptitle)
        plt.xlabel(f"{xlabel} ({VARAXES[0].unit})") # Add units to axis labels
        plt.ylabel(f"{ylabel} ({VARAXES[1].unit})") # Add units to axis labels

        # saveshow() allows the plot to be displayed and saved
        saveshow()
...
#####################################################################################################################################################################################
  ...
  # If the user wants to termiante the session
    ...
    if mainsure in ['y','Y','yes','YES']:
      ...
      print('\n')
      print('****************************************************************************************************************************************',end='\n\n')
      print("Thank you for using GPX (Gravitational Potential Xplorer) !!",end='\n\n')
